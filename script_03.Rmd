...
sep_new <- 
  ...
  filter(u_grad > 10) %>%
  group_by(id, title, name, department) %>%
  summarize(ugrad = sum(u_grad), .groups = "drop")
...
- figure out what we want to know 
- recognize that we cant observe exactly why people decided to drop the courses
- hypothesize why people are dropping these courses
- throwing out irrelevant data, only using data relevant to the time period
...
- understand the limitations to our model 
- understand that there is uncertainty in whatever conclusion we come to 
- because observational and not a study, we can't use a randomized mechanism, 
might be correlation that we can't see because those that are dropping might be
doing so for a reason. not random rep of Harvard student body 
- we cant observe the data on people who almost dropped, only know the 
people that did
...
inner_join(sep_new, sep_old) %>%
  mutate(change = (u_grad - ugrad)) %>%
  arrange(desc(abs(change))) %>%
  slice(1:10) %>%
  ggplot(aes(x = title, y = change, fill = title)) + geom_col() + 
  labs( title = " Top 10 Classes with Greatest Change in Enrollment",
        x = "Course",
        y = "Change in Enrollment") +
  scale_color_manual(name = "Department", values = title) +
  theme_light()
